...
select continent,location,date,max(total_cases) as TotalCases,(max(total_cases)/avg(population))*100 as PeopleInfectedPercentage from CovidDeaths group by continent,location,date;

	-- Total Cases count in each location and it's percentage over population
select location ,avg(population) as Population
       ,SUM(new_cases) as Total_Infection_location
	   --,date
	   ,(sum(new_cases)/avg(population))*100 as Total_Infection_Population_Percentage 
	   from CovidDeaths
	   where continent is not null
	   GROUP BY location--,population,date
	   order by 4 desc;
select location,sum(total_cases) from CovidDeaths group by location;
...
select continent,location ,population
	   ...
	   order by Max_Deaths_Population_Percentage desc;
...
from CovidDeaths
where continent not like '%world' and continent not like 'International'
group by continent;
